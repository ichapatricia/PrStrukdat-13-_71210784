...
class BinaryTree:
    ...
    def inorderOdd(self, node):
        if node != None:
            self.inorder(node.left)
            self.inorder(node.right)

    def sumOdd(self,node,Odd=[]):
        if node != None:
            self.sumOdd(node.left)
            Odd.append(node.data)
            self.sumOdd(node.right)
        return sum(Odd)

    def valueOdd(self):
        hasil = self.sumOdd(self.root.left)
        return "Penjumlahan data ganjil ="+ str(hasil)
